                          ...
                          bool *asset_embed_failed = nullptr)
{
    ...
    j["text_auto_size"] = l.text_auto_size;
    ...
    j["crop_left"]     = aprop_to_json(l.crop_left);
    j["crop_top"]      = aprop_to_json(l.crop_top);
    j["crop_right"]    = aprop_to_json(l.crop_right);
    j["crop_bottom"]   = aprop_to_json(l.crop_bottom);
                                               ...
                                               std::string *error = nullptr)
{
    ...
    l->text_auto_size = json_bool(j, "text_auto_size", false);
    ...
    if (j.contains("crop_left"))   l->crop_left   = aprop_from_json(j["crop_left"],   "crop_left");
    if (j.contains("crop_top"))    l->crop_top    = aprop_from_json(j["crop_top"],    "crop_top");
    if (j.contains("crop_right"))  l->crop_right  = aprop_from_json(j["crop_right"],  "crop_right");
    if (j.contains("crop_bottom")) l->crop_bottom = aprop_from_json(j["crop_bottom"], "crop_bottom");
    ...
    for (auto *crop : {&l->crop_left, &l->crop_top, &l->crop_right, &l->crop_bottom})
        crop->static_value = std::clamp(crop->static_value, 0.0, (double)kMaxCanvasDimension);
