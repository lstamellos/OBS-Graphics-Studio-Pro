...
protected:
    ...
    void mouseMoveEvent(QMouseEvent *event) override
    {
        if (event && (event->buttons() & Qt::LeftButton) && currentItem()) {
            const int distance = (event->pos() - drag_start_pos_).manhattanLength();
                                                   ...
                                                   const QFileInfo &dir_info)
{
    ...
    item->setToolTip(1, obsgs_tr("OBSTitles.EditCategory"));
    item->setToolTip(2, obsgs_tr("OBSTitles.DeleteCategory"));
...
static void populate_template_categories(QTreeWidget *tree, const QString &root_path)
{
    ...
    tree->setHeaderLabels(QStringList{obsgs_tr("OBSTitles.TemplateCategories"), QString(), QString()});
    tree->setColumnWidth(1, 28);
    tree->setColumnWidth(2, 28);
    ...
    QObject::connect(categories, &QTreeWidget::itemClicked, window,
        ...
        if (column == 1) {
            ...
            QString new_name = QInputDialog::getText(
                window, obsgs_tr("OBSTitles.EditCategory"), obsgs_tr("OBSTitles.CategoryNamePrompt"),
                QLineEdit::Normal, item->text(0), &ok).trimmed();
            if (!ok) return;
            new_name = sanitized_template_category_path(new_name);
            if (new_name.isEmpty()) {
                QMessageBox::warning(window, obsgs_tr("OBSTitles.EditCategory"),
                                     obsgs_tr("OBSTitles.TemplateLibraryCategoryRequired"));
                return;
            }
            QFileInfo info(category_path);
            const QString target_path = QDir(info.absolutePath()).filePath(new_name);
            if (QFileInfo::exists(target_path) || !QDir(info.absolutePath()).rename(info.fileName(), new_name)) {
                QMessageBox::warning(window, obsgs_tr("OBSTitles.EditCategory"),
                                     obsgs_tr("OBSTitles.EditCategoryFailed"));
                return;
            }
            populate_template_categories(categories, root_path);
